...
function AttemptPurchase(type, upgradeLevel)
    ...
    if isPurchaseSuccessful then
        ...
        if type == "performance" or type == "repair" or type == "turbo" then
        else
            price = vehicleCustomisationPrices[type].price
            if upgradeLevel then
                mechanicLogs.message = mechanicLogs.message.."\n"..type.." Level ".. upgradeLevel - 1  .. " - $"..price
            else
                mechanicLogs.message = mechanicLogs.message.."\n"..type  .. " - $"..price
            end
            mechanicLogs.price = mechanicLogs.price + price
        end
    end
...
function ExitBennys()
    ...
    if CustomsData.location ~= "Developers" then
        local locationData = Config.Locations[CustomsData.location]
        if locationData then
            TriggerServerEvent('qb-customs:server:sendWebhook', mechanicLogs, locationData.settings.webhook)
        end
    end
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        if currentMechanics >= Config.MinOnlineMechanics and not override and PlayerData.job.name ~= 'bennys' then
            repairOnly = true
        ...
        if locationData then
            mechanicLogs.title = locationData.settings.label.." Player Name: "..name  -- Start mechanicLogs.message
            mechanicLogs.color = locationData.settings.color or 16753920
            
        end
        mechanicLogs.message = ""
        mechanicLogs.price = 0
        if GetVehicleClass(plyVeh) == 8 then --Motorcycle
            isMotorcycle = true
        else
            isMotorcycle = false
...
function SetupInteraction()
    QBCore.Functions.TriggerCallback('qb-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
        ...
        if PlayerData.job.name ~= 'steveauto' and Config.DisableWhenMechanicsOnline and currentMechanics >= Config.MinOnlineMechanics then
...
RegisterNetEvent('qb-customs:client:EnterCustoms', function(override)
    ...
    if not override and next(CustomsData) and CustomsData.location then if not CheckRestrictions(CustomsData.location) then return end end
